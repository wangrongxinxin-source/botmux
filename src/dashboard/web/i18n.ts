...
const zh: DashboardMessages = {
  ...
  'insights.disclosure': '口径：Codex 会话的「读」多走 shell（exec_command），不计入「读」相，故其读改比会偏低甚至为 0；代码改动按 patch 解析，与 Claude 的结构化统计口径不同。跨工具的分布与比值仅供参考，不宜直接对比。',
...
const en: DashboardMessages = {
  ...
  'insights.disclosure': 'Measurement note: Codex reads files mostly via shell (exec_command), not counted as "read", so its read/write ratio runs low or 0; code churn is parsed from patches. Cross-tool distributions and ratios are indicative only, not directly comparable.',
